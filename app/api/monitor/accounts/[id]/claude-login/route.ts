...
export async function POST(_: Request, context: RouteContext) {
  ...
  if (account.provider !== "claude") {
    return NextResponse.json({ ok: false, error: "Claude 계정만 브라우저 로그인을 지원합니다." }, { status: 400 });
  }
  ...
  try {
    ...
    // 브라우저 내부에서 organizations API 호출 (Cloudflare 우회)
    // 브라우저 컨텍스트의 쿠키를 그대로 사용하므로 차단되지 않음
    const verifyResult = await page.evaluate(async () => {
      try {
        ...
        if (!res.ok) return { ok: false, status: res.status, orgs: [] };
        const data = await res.json();
        return { ok: true, status: res.status, orgs: data as Array<{ uuid: string; name?: string }> };
      } catch (e) {
        return { ok: false, status: 0, orgs: [], error: String(e) };
      }
    });
    ...
    if (!verifyResult.ok || !Array.isArray(verifyResult.orgs) || verifyResult.orgs.length === 0) {
      ...
      const updated = await updateMonitorAccount(id, { sessionCookie: cookieString });
      const refreshed = updated.accounts.find((a) => a.id === id);
      return NextResponse.json({
        ok: true,
        message: "로그인 성공 — 쿠키 저장 완료 (조직 정보 조회는 실패)",
        account: refreshed ? toPublicAccount(refreshed) : undefined,
      });
    }

    // 조직 이름에서 이메일 추출 (예: "user@email.com's Organization" → "user@email.com")
    const orgName = verifyResult.orgs[0].name || verifyResult.orgs[0].uuid;
    const emailMatch = orgName.match(/^([^@]+@[^']+)/);
    const displayName = emailMatch ? emailMatch[1] : orgName;

    // 계정에 쿠키 + 이름 저장
    const updated = await updateMonitorAccount(id, { sessionCookie: cookieString, name: displayName });
    const refreshed = updated.accounts.find((a) => a.id === id);
